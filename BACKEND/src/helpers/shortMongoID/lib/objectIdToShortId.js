...
function ObjectIdToShortId(id) {
  ...
  var counter = parseInt(id.toHexString().slice(-10), 16);

  // only use the last 3 digits of the counter to serve as our "milliseconds"
  counter = parseInt(counter.toString().slice(-8), 10);

  // add counter as our millisecond precision to our time
  time = time + counter;

  // convert to 32 base string (not strict base64)
  str = utils.toBase(time, 32);
